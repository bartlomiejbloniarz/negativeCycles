...
void dfs(int v, vector<vector<int>> &invertedNeighbours, Graph &graph, stack<int>& S, vector<bool>&visited, vector<number> &distance) {
    visited[v] = true;
    for (auto neighbour: graph.neighbours[v]) {
        if (distance[v] + neighbour.weight <= distance[neighbour.dst]) {
            invertedNeighbours[neighbour.dst].push_back(v);
            if (!visited[neighbour.dst]) {
                dfs(neighbour.dst, invertedNeighbours, graph, S, visited, distance);
            }
        }
    }
    S.push(v);
...
bool containsNegativeCycle(Graph& graph, int source, number& labelingCount) {
    ...
    while (!topSort.empty()) {
        ...
        for (int src: reachable) {
            if (!visited[src]) {
                dfs(src, invertedNeighbours, graph, sccPass, visited, distance);
            }
        }
